...
SELECT park.name, campground.name, open_from_mm, daily_fee
FROM campground
INNER JOIN park ON campground.park_id = park.park_id
ORDER BY park.name, campground.name;

-- select the park name and the total number of campgrounds for each park ordered by park name
SELECT park.name, COUNT(campground) AS total_campgrounds
FROM park
...
SELECT site_id, reservation.name, from_date, to_date
FROM reservation
