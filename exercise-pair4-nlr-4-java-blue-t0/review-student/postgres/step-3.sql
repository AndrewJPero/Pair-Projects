...
SELECT AVG(visitors) AS average_all_parks
FROM park;
...
SELECT COUNT(utilities) AS total_utilities
FROM site
WHERE utilities = true;
...
FROM reservation
WHERE from_date >= '2021-10-01' AND to_date <= '2021-10-31';
...
SELECT from_date, COUNT(from_date) AS reservation_total
FROM reservation
GROUP BY from_date
ORDER BY from_date;
